...
typedef struct MLXObject {
    ...
    void (*Read)(struct MLXObject *mlx, uint8_t cmd, uint8_t *rData,
                 uint16_t rSize); // 读数据操作指针
    void (*Write)(struct MLXObject *mlx, uint8_t cmd, uint8_t *wData,
                  uint16_t wSize);            // 写数据操作指针
    void (*Delayus)(volatile uint32_t nTime); // 延时操作指针
} MLXObjectType;

/*定义读MLX90614数据操作指针类型*/
typedef void (*MLXRead)(struct MLXObject *mlx, uint8_t cmd, uint8_t *rData,
                        uint16_t rSize);
/*定义写MLX90614数据操作指针类型*/
typedef void (*MLXWrite)(struct MLXObject *mlx, uint8_t cmd, uint8_t *wData,
                         uint16_t wSize);
/*定义微秒延时操作指针类型*/
